...
```
:::{.callout-note}
Python is dynamically typed, meaning that the type of a variable is checked during runtime and not before hand.
...
In essence, they represent an order sequence of values that can have arbitrary types.
```{python}
#| output: false
# List of integers
[2, 4, 6, 8]
...
mean = mean / len(integers)   # len computes the length of a list
print(mean)

# While while loops run until a statement is no longer true
a = 5
b = 2
c = 1
while (a > 0):
...
So we need to talk about scripts files which will lead us to version control.

For now this ends our quick introduction to Python.
