...
In general a package manager (or package-management system) is a tool hat automates the process of:
...
We all use package managers on our smart phones or computers in various forms.
They come in various forms and _accessability_ like the GUIs _Google Play_,  _App Store_,_Microsoft Store_, but also in terminal form as _apt_, _dnf_, _zypper_, _homebrew_, and may more.
Most of these tools give us an easy way to work with some application without needing to know much about how the software is build. The same is true for _software design_.

They especially take care that your modern world of digital infrastructure works even though it often looks like this:
...
Especially cumbersome where problems where you needed different versions of the same library for different projects.
...
It is worth highlighting, that a good package manager can define the difference between a successful programming language and a further note on the [index of programming languages](https://en.wikipedia.org/wiki/List_of_programming_languages).
...
::: {.callout-important}
Please think about what you think are **must have** and **nice to have** features that we should have for our package manager.
:::
...
- must haves
   ...
   - lock files for exact package versions **reproducibility**, see {@sec-intro-pm-reproducibility}
...
## An introduction ot working with PDM for your `python` project
...
>
>Unlike Poetry and Hatch, PDM is not limited to a specific build backend; users have the freedom to choose any build backend they prefer. (Source: [^1])

The following screen cast give us a quick overview of its capabilities.
...
This will prompt you with a couple of questions to and based on these questions the `pyproject.toml` file is initialized.
...
1. We need to specify a project name. This name is used in the `pyproject.toml` and `pdm` will also generate a `src/<name>` directory for you right away (together with a `test` directory). `pdm` uses here its default template, see [docs](https://pdm-project.org/latest/usage/template/).

1. The version of you project needs to be initialized.
...
For now `n` will suffice, but hopefully by the end of this notes you feel comfortable enough to release your own package as soon as this is the case have a look at the [docs](https://pdm-project.org/latest/usage/publish/) and the [Python Packaging User Guide](https://packaging.python.org/).
...
1. Last the python requires is asked.
...
The last highlightes section tell us that `pdm` is designed to work with a _version control system_.
