...
- Create a prey class with the parameters $x, \alpha, \beta$ where only the first is public
- Create a predator class with the parameters $y, \delta, \gamma$ where only the first is public
...
(@) Read in the [death_valley_2014.csv](https://raw.githubusercontent.com/ehmatthes/pcc/refs/heads/master/chapter_16/death_valley_2014.csv) from the GitHub repo corresponding to @Matthes2023. 
    ...
    - Use the `.pipe` function to _apply_ a series of functions to your data. 
        ...
        - Update the `WindDirDegree` such that the values are in $[-180, 180]$ instead of $[0, 360)$
