...
```
::: {.callout-caution}
We currently use the `root` logger and as soon as one handler is defined, i.e. the logger is called for the first time, calling `{.python} logging.basicConfig` has no effect, unless `force=True` is included.
...
The usual convention is to use `__name__`
```{python}
#| echo: false
# Code to consistently have the same output, not shown
import logging
...
```
We can just use it in the same fashion but the output includes the new name (if configured).
:::
