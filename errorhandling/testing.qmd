...
1. make the test folder a module as well
...
(@) `dummy/src/rectangle/__init__.py`
```python
{{< include ../_assets/errorhandling/dummy/src/rectangle/__init__.py >}}
```
(@) `dummy/src/rectangle/rectangle.py`
```python
{{< include ../_assets/errorhandling/dummy/src/rectangle/rectangle.py >}}
```
(@) `dummy/test/rectangle/__init__.py`
```python
{{< include ../_assets/errorhandling/dummy/test/rectangle/__init__.py >}}
```
(@) `dummy/test/rectangle/rectangle.py`
```python
{{< include ../_assets/errorhandling/dummy/test/rectangle/rectangle.py >}}
```
:::

We can run the tests from the terminal (note the change of directory is only necessary for these notes)
```bash
pdm run python -m unittest -v test.rectangle
```
```{python}
#| error: true
#| echo: false
...
cd ../_assets/errorhandling/dummy
pdm run python -m unittest -v test.rectangle
```
...
As there is a compability between `pytest` and `unittest` we can run 
```bash
pdm run pytest -q test/rectangle/rectangle.py
```
```{python}
#| error: true
#| echo: false
